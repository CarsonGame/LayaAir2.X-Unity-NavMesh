...
var mesh;
	...
	var computeCentroids = function (geometry) {
		...
		for ( f = 0, fl = geometry.faces.length; f < fl; f ++ ) {
			...
			face.centroid = new Laya.Vector3( 0, 0, 0 );
			
			add(face.centroid,geometry.vertices[ face.a ],face.centroid);
			add(face.centroid,geometry.vertices[ face.b ],face.centroid);
			add(face.centroid,geometry.vertices[ face.c ],face.centroid);
			scale(face.centroid,1/3,face.centroid);
		}
	};
	...
	_.extend(exports, {
		...
		findPath: function (startPosition, targetPosition, zone, group) {
			...
			_.each(channel.path, function (c) {
				var vec = new Laya.Vector3(c.x, c.y, c.z);
